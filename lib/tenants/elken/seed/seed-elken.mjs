...
async function seed() {
  ...
  const { error: servicesErr } = await supabase
    .from('services')
    .upsert(SERVICES, { onConflict: 'bot_id,name', ignoreDuplicates: false })
  ...
  const { error: faqsErr } = await supabase
    .from('faqs')
    .upsert(FAQS, { onConflict: 'bot_id,question', ignoreDuplicates: false })
  ...
  const { error: productsErr } = await supabase
    .from('products')
    .upsert(PRODUCTS, { onConflict: 'bot_id,name', ignoreDuplicates: false })
